...
public:
 int create_account(int accountNo) {
   ...
   void dep  (int z)      {deposit+=z; }             		    
   void draw (int z)      {deposit-=z; }
           		    
   int retacc()        const { return password; }
   int retac()         const { return accountNo;}
   int retdeposit()    const { return deposit;}

   
   void detail() const { std::cout  <<password<<"       "<<accountNo<<"         "<<name<<"              "<<deposit <<std::endl; }	     

   int admin()   { admpass=007;}
   int readm()   {   if(admpass==007)    {	return admpass;}  }     
}; //class ends here;
